package main

import (
	"bufio"
	"bytes"
	"compress/bzip2"
	"encoding/xml"
	"fmt"
	"io"
	"io/ioutil"
	"os"
	"strconv"
	"strings"
	"sync"

	"github.com/dgraph-io/badger/v3"
	"github.com/willbeason/extract-wikipedia/pkg/documents"
	"github.com/willbeason/extract-wikipedia/pkg/flags"
	"github.com/willbeason/extract-wikipedia/pkg/jobs"

	"github.com/spf13/cobra"
)

func mainCmd() *cobra.Command {
	cmd := &cobra.Command{
		Args: cobra.ExactArgs(3),
		Use: `extract-wikipedia path/to/pages-articles-multistream.xml.bz2 \
  path/to/pages-articles-multistream-index.txt \
  path/to/output.db`,
		Short: `Extracts the compressed pages-articles-multistream dump of Wikipedia to an output
Badger database, given an already-extracted index file.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			nParallel, err := cmd.Flags().GetInt(flags.ParallelKey)
			if err != nil {
				return err
			}

			cmd.SilenceUsage = true

			repo := args[0]
			index := args[1]
			outDB := args[2]

			fRepo, err := os.Open(repo)
			if err != nil {
				return err
			}
			defer func() {
				err = fRepo.Close()
			}()

			fIndex, err := os.Open(index)
			if err != nil {
				return err
			}
			defer func() {
				_ = fIndex.Close()
			}()

			rIndex := bufio.NewReader(fIndex)

			errs, errsWg := jobs.Errors()

			jobFiles := make(chan job)

			jobSync := sync.WaitGroup{}
			jobSync.Add(1)
			go func() {
				extractFile(rIndex, fRepo, jobFiles, errs)

				close(jobFiles)
				jobSync.Done()
			}()

			db, err := badger.Open(badger.DefaultOptions(outDB))
			defer func() {
				_ = db.Close()
			}()

			wg := sync.WaitGroup{}
			for w := 0; w < nParallel; w++ {
				wg.Add(1)
				go func() {
					worker(db, jobFiles, errs)
					wg.Done()
				}()
			}

			jobSync.Wait()
			wg.Wait()
			close(errs)
			errsWg.Wait()

			err = jobs.RunGC(db)
			if err != nil {
				return err
			}

			err = db.Close()
			if err != nil {
				return err
			}

			return fIndex.Close()
		},
	}

	flags.Parallel(cmd)

	return cmd
}

func worker(db *badger.DB, work <-chan job, errs chan<- error) {
	for j := range work {
		decompress(j.b, db, errs)
	}
}

type job struct {
	b []byte
}

var (
	nPages    = 0
	nPagesMtx = sync.Mutex{}
)

func normalize(text string) string {
	if !strings.HasPrefix(text, "<mediawiki") {
		text = "<mediawiki>\n" + text
	}

	if !strings.HasSuffix(text, "</mediawiki>") {
		text += "\n</mediawiki>\n"
	}
	return text
}

func decompress(b []byte, db *badger.DB, errs chan<- error) {
	bz := bzip2.NewReader(bytes.NewReader(b))

	b, err := ioutil.ReadAll(bz)
	if err != nil {
		errs <- err
		return
	}

	text := normalize(string(b))

	doc := &documents.XMLDocument{}

	err = xml.Unmarshal([]byte(text), doc)
	if err != nil {
		errs <- err
		return
	}

	for _, page := range doc.Pages {
		if page.NS != 0 || page.Redirect.Title != "" {
			// Ignore redirects and non-articles.
			continue
		}

		err = db.Update(jobs.WriteProto(page.ToProto()))
		if err != nil {
			errs <- err
		}
		nPagesMtx.Lock()
		if nPages%10000 == 0 {
			fmt.Printf("%d Pages\n", nPages)

			if nPages%100000 == 0 {
				err = jobs.RunGC(db)
				if err != nil {
					errs <- err
				}
			}
		}
		nPages++
		nPagesMtx.Unlock()
	}

	if err != nil {
		errs <- err
	}
}

func main() {
	err := mainCmd().Execute()
	if err != nil {
		os.Exit(1)
	}
}

func extractFile(rIndex *bufio.Reader, fRepo *os.File, work chan<- job, errs chan<- error) {
	startIndex, endIndex := int64(0), int64(0)

	var (
		lineBytes []byte
		err       error
		outBytes  []byte
	)

	for ; err == nil; lineBytes, _, err = rIndex.ReadLine() {
		if len(lineBytes) == 0 {
			continue
		}

		line := string(lineBytes)
		parts := strings.SplitN(line, ":", 3)

		endIndex, err = strconv.ParseInt(parts[0], 10, strconv.IntSize)

		if err != nil {
			errs <- err
			return
		}

		if startIndex == endIndex {
			continue
		}

		outBytes = make([]byte, endIndex-startIndex)

		_, err = fRepo.Read(outBytes)
		if err != nil {
			errs <- err
			return
		}

		work <- job{b: outBytes}

		startIndex = endIndex

		if err == io.EOF {
			break
		}
	}

	if err == io.EOF {
		fmt.Println("got last file")

		outBytes, err = ioutil.ReadAll(fRepo)
		if err != nil {
			errs <- err
			return
		}
		work <- job{b: outBytes}
	}
}
