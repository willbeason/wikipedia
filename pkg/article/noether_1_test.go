...
:_math_ 
where a matrix (or linear transform)  (without the variable ) times a vector _math_ (that only has non-zero powers of ) is equal to the zero vector, . Hence, the determinant of the matrix  must be zero, providing a new equation in which the variable  has been eliminated.

====Invariant theory of finite groups====
Techniques such as Hilbert's original non-constructive solution to the finite basis problem could not be used to get quantitative information about the invariants of a group action, and furthermore, they did not apply to all group actions. In her 1915 paper, Noether found a solution to the finite basis problem for a finite group of transformations  acting on a finite-dimensional vector space over a field of characteristic zero. Her solution shows that the ring of invariants is generated by homogeneous invariants whose degree is less than, or equal to, the order of the finite group; this is called '''Noether's bound'''. Her paper gave two proofs of Noether's bound, both of which also work when the characteristic of the field is coprime to _math_ (the factorial of the order _math_ of the group ). The degrees of generators need not satisfy Noether's bound when the characteristic of the field divides the number _math_, but Noether was not able to determine whether this bound was correct when the characteristic of the field divides _math_ but not _math_. For many years, determining the truth or falsehood of this bound for this particular case was an open problem, called "Noether's gap". It was finally solved independently by Fleischmann in 2000 and Fogarty in 2001, who both showed that the bound remains true.
