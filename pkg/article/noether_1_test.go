...
thumb|upright=1.2|Noether sometimes used postcards to discuss abstract algebra with her colleague, Ernst Fischer. This card is postmarked 10&nbsp;April 1915.
...
thumb|left|upright|In 1915 David Hilbert invited Noether to join the Göttingen mathematics department, challenging the views of some of his colleagues that a woman should not be allowed to teach at a university.
...
thumb|210px|The mathematics department at the University of Göttingen allowed Noether's ''habilitation'' in 1919, four years after she had begun lecturing at the school.
...
thumb|230x230px|B.L. van der Waerden (pictured in 1980) was heavily influenced by Noether during his time in Göttingen.
...
thumb|Noether taught at Moscow State University during the winter of 1928–1929.|left
...
thumb|right|Bryn Mawr College provided a welcoming home for Noether during the last two&nbsp;years of her life.
...
==Death==
thumb|250px|right|Noether's ashes were placed under the walkway surrounding the cloisters of Bryn Mawr's M. Carey Thomas Library.
...
A ''ring'' likewise, has a set of elements, but now has ''two'' operations. The first operation must make the set a commutative group, and the second operation is associative and distributive with respect to the first operation. It may or may not be commutative; this means that the result of applying the operation to a first and a second element is the same as to the second and first – the order of the elements does not matter. If every non-zero element has a multiplicative inverse (an element  such that ), the ring is called a ''division ring''. A ''field'' is defined as a commutative division ring. For instance, the integers form a commutative ring whose elements are the integers, and the combining operations are addition and multiplication. Any pair of integers can be added or multiplied, always resulting in another integer, and the first operation, addition, is commutative, i.e., for any elements  and  in the ring, . The second operation, multiplication, also is commutative, but that need not be true for other rings, meaning that  combined with  might be different from  combined with . Examples of noncommutative rings include matrices and quaternions. The integers do not form a division ring, because the second operation cannot always be inverted; for example, there is no integer  such that .
...
An example of an ''invariant'' is the discriminant  of a binary quadratic form , where  and  are vectors and "''''''" is the dot product or "inner product" for the vectors. , , and  are linear operators on the vectors – typically matrices.

The discriminant is called "invariant" because it is not changed by linear substitutions  and  with determinant . These substitutions form the special linear group .

One can ask for all polynomials in , , and  that are unchanged by the action of ; these are called the invariants of binary quadratic forms and turn out to be the polynomials in the discriminant.

More generally, one can ask for the invariants of homogeneous polynomials  of higher degree, which will be certain polynomials in the coefficients , and more generally still, one can ask the similar question for homogeneous polynomials in more than two variables.
...
:<math> \mathbf{Mv} = \mathbf{0}</math> 
where a matrix (or linear transform)  (without the variable ) times a vector <math>\mathbf{v}</math> (that only has non-zero powers of ) is equal to the zero vector, . Hence, the determinant of the matrix  must be zero, providing a new equation in which the variable  has been eliminated.
...
====Topology====
thumb|right|250px|A continuous deformation (homotopy) of a coffee cup into a doughnut (torus) and back
...
* 
* 
*[https://web.archive.org/web/20070929100418/http://www.physikerinnen.de/noetherlebenslauf.html Noether's application for admission to the University of Erlangen and three of her curriculum vitae] from the website of historian Cordula Tollmien
*[https://triarte.brynmawr.edu/objects-1/info/188672 Letter by Noether to Dr. Park, Bryn Mawr College President] – Bryn Mawr College Library Special Collections
*[https://triarte.brynmawr.edu/objects-1/info/188671 Photograph of Noether taken by Hanna Kunsch] – Bryn Mawr College Library Special Collections
*[https://owpdb.mfo.de/search?term=noether Photographs of Noether] – Oberwolfach Photo Collection of the Mathematisches Forschungsinstitut Oberwolfach
*[https://faculty.evansville.edu/ck6/bstud/enmc.html Photographs of Noether's colleagues and acquaintances] from the website of Clark Kimberling
